...
export function buildTray(storage: Storage, getMainWindow: () => BrowserWindow | null, syncAll: () => Promise<void>): TrayController {
  ...
  const blank16 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAIAAACQkWg2AAAAEElEQVR4nGNgGAWjYBSAAAADCgABZl1cZAAAAABJRU5ErkJggg==';
  ...
  function rebuild(): void {
    ...
    const states: AccountState[] = accounts.map((a) =>
      computeAccountState({ account: a, entries: storage.listEntries(a.id), historicalEntries: storage.listEntries(a.id) }),
    );
    states.sort((a, b) => b.deltaPct - a.deltaPct); // worst (most behind) first

    const accountItems = states.length === 0
    ...
    const menu = Menu.buildFromTemplate([
      ...
      { label: 'Ouvrir', click: () => { const w = getMainWindow(); if (w) { w.show(); w.focus(); } } },
      { type: 'separator' },
  ...
  tray.on('double-click', () => { const w = getMainWindow(); if (w) { w.show(); w.focus(); } });
  rebuild();
...
function statusEmoji(s: AccountState['status']): string {
  switch (s) {
    ...
    case 'ahead': return '🔵';
    case 'behind': return '🟡';
    case 'over_quota': return '🔴';
    case 'period_ended': return '⚪';
  }
}
